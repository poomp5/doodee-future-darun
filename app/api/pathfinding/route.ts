...
function buildPrompt(profile: ProfileSummary, targetField: string, targetUniversity?: string): string {
...
รางวัล/เกียรติบัตร: ${profile.achievements.length > 0 ? profile.achievements.slice(0, 5).join(", ") : "ไม่ระบุ"}
...
export async function POST(request: Request) {
  try {
    ...
    const body: { targetField?: string; targetUniversity?: string } = await request.json().catch(() => ({}));
    ...
    const prompt = buildPrompt(profile, targetField, targetUniversity);
